...
async function notify(message) {
  ...
  if (message.type == "clip") {
    ...
    const article = message.article ? JSON.parse(message.article) : await getArticleFromDom(message.dom);
...
async function ensureScripts(tabId) {
  const results = await browser.scripting.getRegisteredContentScripts({ids: ["contentScript"]})
